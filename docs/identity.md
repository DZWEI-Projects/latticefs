...
NeuralFS uses **capability-based security** (UCAN tokens). To issue and validate capabilities, the CLI needs a cryptographic identity. The CLI uses an Ed25519 keypair as its local identity.
...
If the keyring isn’t available, NeuralFS can fall back to an encrypted file store (Argon2id + AES‑GCM).
