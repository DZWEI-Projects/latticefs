...
These walkthroughs show end‑to‑end flows with NeuralFS. Each one is written as a small story that explains *why* you are running a command and what it changes in the system. The central idea is simple: **you retrieve and organize data by meaning**, not by file paths or filenames.
...
1. Add files with a user tag to group them at a high level. You are telling NeuralFS that both files belong to a personal music collection:
...
At this point, the files are stored as objects. NeuralFS also extracts ID3 tags automatically, so it already knows artist, album, track number, and other context.

1. Inspect extracted metadata. This lets you confirm what the system learned from the file:
...
1. Create a view scoped by the ID3 artist tag. This is where the workflow departs from the traditional filesystem: you stop thinking about where a file lives, and you ask for what it *is*:
...
1. Export the view to a directory. Exporting materializes the query result so other tools can consume it:
...
1. Revise content. This creates a new version under the same object ID, preserving history:
...
1. Mark the latest version as review. State is a workflow signal, not a content change:
...
1. Approve after review. This is the moment where you decide the version is safe to share:

```bash
lsf state set <object-id> approved
```

1. Create a view of approved documents. The view captures intent in a single query:
...
1. Export the view. This produces a shareable directory without changing the underlying data model:
...
1. Add a script. NeuralFS auto‑detects executables on Unix and tags them accordingly:

```bash
lsf add ./scripts/scan.sh --tag project:security
```

NeuralFS tags executables automatically, which makes policy and review flows easier. You do not have to remember which files were “dangerous.”

1. Quarantine it. This records explicit caution in metadata without moving the file anywhere:
...
1. If it passes review, update trust and seal the version. Sealing is a hard lock:
...
1. Any attempt to create a new version now fails, which protects the object from accidental changes:
