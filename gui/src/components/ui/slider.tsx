...
>(({ className, value, defaultValue, min = 0, max = 100, onValueChange, ...props }, ref) => {
  ...
  const resolvedValue = (isControlled ? value : internalValue) ?? defaultValue ?? [min];
  const currentValue = resolvedValue[0] ?? min;
  const range = max - min;
  const ratio = range === 0 ? 0 : (currentValue - min) / range;
  const clampedRatio = Math.min(1, Math.max(0, ratio));
  const angle = 225 + 270 * clampedRatio;
  const radians = (angle * Math.PI) / 180;
  const radius = 28;
  const orbX = 50 + radius * Math.cos(radians);
  const orbY = 50 + radius * Math.sin(radians);
    ...
    >
      ...
      <SliderPrimitive.Thumb
        ...
        className={cn(
          ...
          "after:bg-[radial-gradient(circle_at_var(--orb-x)_var(--orb-y),rgba(255,255,255,0.75),transparent_60%)] after:opacity-90",
