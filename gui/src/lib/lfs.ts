...
export const getVersionContent = async (
  ...
  if (isTauriApp()) {
    return invoke<string | null>("get_version_content", {
      objectId,
      versionId,
    });
  }
...
export const reviseObject = async (
  ...
  if (isTauriApp()) {
    const o = await invoke<{
      ...
      current_version_state: string;
      is_sealed: boolean;
    }>("revise_object", { objectId, content, message: message ?? null });
    return {
      id: o.id,
      name: o.name,
      extension: o.extension,
...
export const diffVersions = async (
  ...
  if (isTauriApp()) {
    const d = await invoke<{
      ...
      right_size: number;
      identical: boolean;
    }>("diff_versions", { objectId, versionIdA, versionIdB });
    return {
...
export const exportObjectVersion = async (
  ...
  if (isTauriApp()) {
    // If no output path provided, use a save dialog
    if (!outputPath) {
